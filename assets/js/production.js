...
;$toolsetcustomposttypes = jQuery.noConflict();

(function($) {
	$(function() {

		// Set menu state
		if (window.toolset_menu_parent_id && window.toolset_menu_page_href) {
			$('#' + window.toolset_menu_parent_id).addClass('wp-current-submenu').addClass('wp-has-current-submenu');
			$('#' + window.toolset_menu_parent_id + ' .menu-top-first').addClass('wp-has-current-submenu');
			$('a[href="admin.php?page=' + window.toolset_menu_page_href + '"]').parents('li').addClass('current');
			$('a[href="admin.php?page=' + window.toolset_menu_page_href + '"]').addClass('current');
		}

		var context = $('.toolset_custom_post_types');

		// Show in menu
		$('.toolset_custom_post_types_show_in_menu input').click(function() {
			$(this).parents('.toolset_custom_post_types_show_in_menu').find('select').val('');
		});

		// Dashicons select
		$('.toolset_custom_post_types_cpt_input_wrapper.icons input').change(function() {
			$(this).parents('.toolset_custom_post_types_cpt_input_wrapper.icons').find('label').removeClass('checked');
			$(this).parents('label').addClass('checked');
		});

		// Delete checkbox
		$('.toolset_custom_post_types_delete_checkbox').click(function() {
			var parent = $(this).parents('.toolset_custom_post_types_options_content');
			if ($(this).is(':checked')) {
				parent.find('.toolset_custom_post_types_cpt_option, .toolset_custom_post_types_separator, h4').not($(this).parents('.toolset_custom_post_types_cpt_option')).hide();
				$(this).parents('.toolset_custom_post_types_wrapper').find('.will_be_deleted').show();
			} else {
				parent.find('.toolset_custom_post_types_cpt_option, .toolset_custom_post_types_separator, h4').not($(this).parents('.toolset_custom_post_types_cpt_option')).show();
				$(this).parents('.toolset_custom_post_types_wrapper').find('.will_be_deleted').hide();
			}
		});

	});
})($toolsetcustomposttypes);
